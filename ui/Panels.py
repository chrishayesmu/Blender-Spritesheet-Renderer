...
class ReportingPanel(bpy.types.Panel):
    ...
    def poll(self, context):
        return context.scene.ReportingPropertyGroup.hasAnyJobStarted
    ...
    def draw(self, context):
        ...
        if reportingProps.jobInProgress:
            ...
            timeRemainingStr = StringUtil.timeAsString(timeRemaining) if timeRemaining != None else "---"
            ...
            if not reportingProps.lastErrorMessage and reportingProps.fileExplorerType not in ("none", "unknown"):
                row = self.layout.row()
                op = row.operator("spritesheet.opendirectory", text = "Open Last Job Output")
...
class ScenePropertiesPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if Render.SpritesheetRenderModalOperator.renderDisabledReason:
            ...
            if "addon preferences" in Render.SpritesheetRenderModalOperator.renderDisabledReason.lower():
                row = box.row()
