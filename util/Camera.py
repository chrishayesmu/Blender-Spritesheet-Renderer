...
def _optimize_for_action(context, camera, target_object, action):
...
def _optimize_for_all_frames(context, camera, target_object, rotation_root, rotations_degrees, actions):
    if camera.data.type != "ORTHO":
...
def _optimize_for_rotation(context, camera, target_object, rotation_root, rotation_degrees, actions):
    if camera.data.type != "ORTHO":
...
def _find_bounding_box_for_action(context, camera, target_object, action):
...
def _get_camera_image_plane(camera):
    ...
    size_vec.rotate(camera.rotation_quaternion)

    return Bounds.from_center_and_size(camera.location, size_vec)

def _select_only_object(scene, target_object):
